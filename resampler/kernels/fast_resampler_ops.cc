...
#include "resampler/kernels/fast_resampler_ops.h"
...
template <typename T>
struct FastResampler2DFunctor<CPUDevice, T> {
    ...
    auto resample_batches = [&](const int start, const int limit) {
      for (int batch_id = start; batch_id < limit; ++batch_id) {
        ...
        auto get_data_point = [&](const int x, const int y, const int chan) {
          return data[batch_id * data_batch_stride +
        ...
        for (int sample_id = 0; sample_id < num_sampling_points; ++sample_id) {
	      ...
	      const T one_minus_dx = one - dx;
	      const T one_minus_dy = one - dy;

	      for (int chan = 0; chan < data_channels; ++chan) {
	        const T img_fxfy = dx * dy * get_data_point(fx, fy, chan);
	        const T img_cxcy = (one_minus_dx) * (one_minus_dy) * get_data_point(cx, cy, chan);
	        const T img_fxcy = dx * (one_minus_dy) * get_data_point(fx, cy, chan);
	        const T img_cxfy = (one_minus_dx) * dy * get_data_point(cx, fy, chan);
	        output[batch_id * output_batch_stride + sample_id * data_channels +
                 chan] = img_fxfy + img_cxcy + img_fxcy + img_cxfy;
	      }
        }
      }
    };
...
class FastResamplerOp : public ::tensorflow::OpKernel {
 public:
  explicit FastResamplerOp(::tensorflow::OpKernelConstruction* context)
  ...
  void Compute(::tensorflow::OpKernelContext* ctx) override {
    ...
    if (num_sampling_points > 0) {
      functor::FastResampler2DFunctor<Device, T>()(
 ...
 private:
  TF_DISALLOW_COPY_AND_ASSIGN(FastResamplerOp);
};

#define REGISTER(TYPE)                                                \
  REGISTER_KERNEL_BUILDER(                                            \
      Name("FastResampler").Device(DEVICE_CPU).TypeConstraint<TYPE>("T"), \
      FastResamplerOp<CPUDevice, TYPE>);
...
#define REGISTER(TYPE)                                                \
  REGISTER_KERNEL_BUILDER(                                            \
      Name("FastResampler").Device(DEVICE_GPU).TypeConstraint<TYPE>("T"), \
      FastResamplerOp<GPUDevice, TYPE>)
TF_CALL_half(REGISTER);
TF_CALL_float(REGISTER);
TF_CALL_double(REGISTER);
#undef REGISTER
...
template <typename T>
struct FastResamplerGrad2DFunctor<CPUDevice, T> {
  void operator()(::tensorflow::OpKernelContext* ctx, const CPUDevice& d,
                  const T* __restrict__ data, const T* __restrict__ warp,
        ...
        for (int sample_id = 0; sample_id < num_sampling_points; ++sample_id) {
          ...
          const int fx = std::floor(static_cast<float>(x));
            ...
            for (int chan = 0; chan < data_channels; ++chan) {
              ...
              update_grad_warp(
                  sample_id, 0, grad_output_value * (one_minus_dy * (img_cxcy - img_fxcy) +
                  dy * (img_cxfy - img_fxfy)));

              update_grad_warp(
                  sample_id, 1, grad_output_value * ((one_minus_dx) * (img_cxcy - img_cxfy) +
                  dx * (img_fxcy - img_fxfy)));

              update_grad_data(fx, fy, chan, grad_output_value * dx * dy);
              update_grad_data(cx, cy, chan, grad_output_value * (one_minus_dx) * one_minus_dy);
              update_grad_data(fx, cy, chan, grad_output_value * dx * one_minus_dy);
              update_grad_data(cx, fy, chan, grad_output_value * (one_minus_dx) * dy);
...
template <typename Device, typename T>
class FastResamplerGradOp : public ::tensorflow::OpKernel {
 public:
  explicit FastResamplerGradOp(::tensorflow::OpKernelConstruction* context)
      : ::tensorflow::OpKernel(context) {}
  ...
  void Compute(::tensorflow::OpKernelContext* ctx) override {
    ...
    // Execute kernel only for nonempty output; otherwise Eigen crashes on GPU.
    if (num_sampling_points > 0) {
      functor::FastResamplerGrad2DFunctor<Device, T>()(
          ctx, ctx->eigen_device<Device>(), data.flat<T>().data(),
 ...
 private:
  TF_DISALLOW_COPY_AND_ASSIGN(FastResamplerGradOp);
};

#define REGISTER(TYPE)                                                    \
  REGISTER_KERNEL_BUILDER(                                                \
      Name("FastResamplerGrad").Device(DEVICE_CPU).TypeConstraint<TYPE>("T"), \
      FastResamplerGradOp<CPUDevice, TYPE>);
...
#define REGISTER(TYPE)                                                    \
  REGISTER_KERNEL_BUILDER(                                                \
      Name("FastResamplerGrad").Device(DEVICE_GPU).TypeConstraint<TYPE>("T"), \
      FastResamplerGradOp<GPUDevice, TYPE>)
