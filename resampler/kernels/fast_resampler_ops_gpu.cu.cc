...
template <typename T>
__global__ void FastResampler2DKernel(const T* __restrict__ data,
  ...
  for(int index: CudaGridRangeX(output_data_size)) {
    ...
    const T img_fxfy = dx * dy * GET_DATA_POINT(fx, fy);
    const T img_cxcy = one_minus_dx * one_minus_dy * GET_DATA_POINT(cx, cy);
    const T img_fxcy = dx * one_minus_dy * GET_DATA_POINT(fx, cy);
    const T img_cxfy = one_minus_dx * dy * GET_DATA_POINT(cx, fy);
...
template <typename T>
struct FastResampler2DFunctor<GPUDevice, T> {
  void operator()(::tensorflow::OpKernelContext* ctx, const GPUDevice& d,
    ...
    TF_CHECK_OK(CudaLaunchKernel(
        FastResampler2DKernel<T>, config.block_count, config.thread_per_block, 0,
...
template struct FastResampler2DFunctor<GPUDevice, Eigen::half>;
template struct FastResampler2DFunctor<GPUDevice, float>;
template struct FastResampler2DFunctor<GPUDevice, double>;
...
template <typename T>
__global__ void FastResamplerGrad2DKernel(
    const T* __restrict__ data, const T* __restrict__ warp,
  ...
  const int resampler_output_size =
      batch_size * num_sampling_points * data_channels;
  for(int index: CudaGridRangeX(resampler_output_size)) {
    ...
    const T img_fxfy = GET_DATA_POINT(fx, fy);
    const T img_cxcy = GET_DATA_POINT(cx, cy);
    const T img_fxcy = GET_DATA_POINT(fx, cy);
    const T img_cxfy = GET_DATA_POINT(cx, fy);
    ...
    atomicAdd(grad_warp + warp_id_x,
             grad_output_value * (one_minus_dy * (img_cxcy - img_fxcy) +
                                     dy * (img_cxfy - img_fxfy)));
    atomicAdd(grad_warp + warp_id_y,
                grad_output_value * (one_minus_dx * (img_cxcy - img_cxfy) +
                                     dx * (img_fxcy - img_fxfy)));

    UPDATE_GRAD_DATA_POINT(fx, fy, grad_output_value * dx * dy);
    UPDATE_GRAD_DATA_POINT(cx, cy, grad_output_value * one_minus_dx * one_minus_dy);
    UPDATE_GRAD_DATA_POINT(fx, cy, grad_output_value * dx * one_minus_dy);
    UPDATE_GRAD_DATA_POINT(cx, fy, grad_output_value * one_minus_dx * dy);
...
template <typename T>
struct FastResamplerGrad2DFunctor<GPUDevice, T> {
  void operator()(::tensorflow::OpKernelContext* ctx, const GPUDevice& d,
                  const T* __restrict__ data, const T* __restrict__ warp,
                  const T* __restrict__ grad_output, T* __restrict__ grad_data,
    ...
    TF_CHECK_OK(CudaLaunchKernel(FastResamplerGrad2DKernel<T>, config.block_count,
...
template struct FastResamplerGrad2DFunctor<GPUDevice, float>;
